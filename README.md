...
Pipeline de datos y dashboard de matriculaciones de vehículos en España, construido sobre los **microdatos públicos de la DGT** y replicando la metodología de clasificación de Simmix (canales, subcanales, segmentos, combustible) para independizarse del proveedor.
...
data/processed/dgt_canal_*.csv · dgt_prov_*.csv · dgt_alerts_*.csv
   │  scripts/build_dashboard_data.py  ← agregados + drift vs Simmix
   ▼
...
| `validation/` | Exports Simmix `BBDD_*` (gitignored) y deltas de validación |
...
La ETL replica la clasificación Simmix desde el dato bruto DGT. Reglas principales, documentadas en `docs/`:

1. **Scope**: turismos M1 + furgonetas ligeras N1, más rescate N2 de derivados de furgoneta que Simmix incluye (Renault Trucks Master, MAN TGE, Fuso Canter, Isuzu serie N, Iveco Daily).
...
4. **Carroceros→chasis**: los carrozados/camperizados (Benimar, Erke, Sortimo…) se atribuyen a la marca/modelo del chasis (Ducato→Fiat, Transit→Ford, TGE→MAN…), como hace Simmix. Los no mapeados generan alerta `CARROCERO_UNMAPPED`.
5. **Combustible**: propulsión DGT + categoría eléctrica; HEV cuenta como ICE; MHEV por texto de versión.
6. **Segmento/Body/Focus**: maestro propio derivado (`public/data/simmix_model_lookup.json` + `masters/`), con cola de modelos nuevos vía alertas.
...
Con cada export Simmix nuevo: dejarlo en `validation/`, reprocesar y revisar `public/data/simmix_drift.json` (o los `delta_*.csv` de `validation/`). Las alertas de drift diarias salen en `data/processed/dgt_alerts_*.csv`.
